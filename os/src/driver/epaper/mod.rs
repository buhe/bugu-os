...
fn io_mux_init() {
    ...
    fpioa::set_function(io::LCD_RST, fpioa::function::gpiohs(epaper::RST_GPIONUM));
    fpioa::set_io_pull(io::LCD_RST, fpioa::pull::DOWN); // outputs must be pull-down
    fpioa::set_function(io::LCD_DC, fpioa::function::gpiohs(epaper::DCX_GPIONUM));
...
lazy_static! {
    ...
    pub static ref DRIVER: Mutex<LCD<SPIImpl<SPI0>>> = {
        ...
        lcd.init();
        lcd.set_direction(epaper::direction::YX_RLDU);
        Mutex::new(lcd)
    };
}
